...
class plugin_mw_cai_syntaxhighlighter {
	...
	public function __construct() {
		...
		$this->pluginurl = $_G['siteurl'].'source/plugin/mw_cai_syntaxhighlighter/';
		...
		$this->brush_files = array(
			...
			"Jass" => array('shBrushJass.js', 'jass', '2.1', 'true'),
		...
		if($_G['cache']['plugin']['mw_cai_syntaxhighlighter']['libversion'] == 2) {
		...
		$this->theme = $_G['cache']['plugin']['mw_cai_syntaxhighlighter']['theme'] ? $_G['cache']['plugin']['mw_cai_syntaxhighlighter']['theme'] : 'Random';//note 没有选择则随机显示
		$this->select_language = unserialize($_G['cache']['plugin']['mw_cai_syntaxhighlighter']['language']);
		$this->autolinks = $_G['cache']['plugin']['mw_cai_syntaxhighlighter']['autolinks'];
		$this->quickcodecopy = $_G['cache']['plugin']['mw_cai_syntaxhighlighter']['quickcodecopy'];
		$this->gutter = $_G['cache']['plugin']['mw_cai_syntaxhighlighter']['gutter'];
		$this->startlinenumber = intval($_G['cache']['plugin']['mw_cai_syntaxhighlighter']['startlinenumber']);
		$this->linenumberpadding = $_G['cache']['plugin']['mw_cai_syntaxhighlighter']['linenumberpadding'];
		$this->toolbar = $_G['cache']['plugin']['mw_cai_syntaxhighlighter']['toolbar'];
		$this->collapse = $_G['cache']['plugin']['mw_cai_syntaxhighlighter']['collapse'];
		$this->smarttabs = $_G['cache']['plugin']['mw_cai_syntaxhighlighter']['smarttabs'];
		$this->tabsize = intval($_G['cache']['plugin']['mw_cai_syntaxhighlighter']['tabsize']);
		$this->wraplines = $_G['cache']['plugin']['mw_cai_syntaxhighlighter']['wraplines'];
		$this->title = str_replace('\'', '&#39;', $_G['cache']['plugin']['mw_cai_syntaxhighlighter']['title']);
		$this->maxheight = $_G['cache']['plugin']['mw_cai_syntaxhighlighter']['maxheight'];
		$this->legacy_enable = $_G['cache']['plugin']['mw_cai_syntaxhighlighter']['legacy'];//note 暂时未做表单提交
		if($this->legacy_enable == 1) {
			$this->legacy = "dp.SyntaxHighlighter.HighlightAll('code')";
		} else {
			$this->legacy = '';
		}
		$this->collapse_lable_text = str_replace('\'', '&#39;', $_G['cache']['plugin']['mw_cai_syntaxhighlighter']['collapse_lable_text']);//note 暂时未做表单提交
	...
	function global_footer() {
		...
		if(CURMODULE == 'viewthread') {
			include_once template('mw_cai_syntaxhighlighter:module');
		...
		if(CURMODULE == 'viewthread' || CURMODULE == 'forumdisplay' || CURMODULE == 'post') {
			...
			$codbox_script = mw_cai_syntaxhighlighter_shl_codebox($sh_lang, $this->gutter).
					'<script type="text/javascript" src="source/plugin/mw_cai_syntaxhighlighter/js/mw_cai_syntaxhighlighter.js?'.VERHASH.'"></script>';
		}
...
font-family: "Consolas", "Bitstream Vera Sans Mono", "Courier New", Courier, monospace !important; /* Set the font type */
	...
	function _syntaxhighlighter_load_footer_script() {
		...
		$script = "\n<!-- mw_cai_syntaxhighlighter ver.".$this->libversion." Begin -->\n";
	...
	".$this->legacy."
//]]></script>
<!-- mw_cai_syntaxhighlighter ver.".$this->libversion." End -->\n";
		} elseif($this->libversion == '2.1.382') {
			$script.= "<script type=\"text/javascript\">//<![CDATA[
...
<!-- mw_cai_syntaxhighlighter ver.".$this->libversion." End -->\n";
	...
	function _mw_shl_codebox_script($textareaid) {
		return '<script type="text/javascript" reload="1">mw_cai_syntaxhighlighter("'.$textareaid.'");</script>';
	}

	function discuzcode($param) {
		global $_G;
		if($param['caller'] == 'discuzcode') {
			$_G['discuzcodemessage'] = preg_replace('/\[mw_shl_code=(.+?),(.+?)\](.*?)\[\/mw_shl_code\]/ies', "\$this->_mw_shl_code_to_pre('\\1', '\\2', '\\3')", $_G['discuzcodemessage']);
		}
		if($param['caller'] == 'messagecutstr') {
			$_G['discuzcodemessage'] = preg_replace('/\[mw_shl_code=(.+?)\](.*?)\[\/mw_shl_code\]/ies', '', $_G['discuzcodemessage']);
		}
	}

	function _mw_shl_code_to_pre($brush, $gutter, $code) {
		global $_G, $post;
		...
		$toolbarstr = '<div style="font-size:12px;">['.$sh_lang[$brush][0].'] <em class="viewsource" style="cursor:pointer;font-size:12px;color:#369 !important;">'.lang('plugin/mw_cai_syntaxhighlighter', 'syntaxhighlighter_viewsource').'</em> <em class="copycode" style="cursor:pointer;font-size:12px;color:#369 !important;">'.lang('plugin/mw_cai_syntaxhighlighter', 'syntaxhighlighter_copycode').'</em></div>';
		return '<div style="padding:15px 0;">'.$toolbarstr.'<pre class="brush: '.strtolower($brush).'; gutter: '.$gutter.'">'.$code.'</pre></div>';
	}

}

class plugin_mw_cai_syntaxhighlighter_forum extends plugin_mw_cai_syntaxhighlighter {

	function viewthread_bottom_output() {
		global $postlist;
		foreach($postlist as $pid => $post) {
			$post['message'] = preg_replace('/<pre[^>](.*?)>(.*?)<\/pre>/ies', "\$this->_brreplace('\\1', '\\2', $pid)", $post['message']);
			$postlist[$pid] = $post;
		}
		return '';
	}
